...
// metrics implements the observability surface of DESIGN.md §8.
type metrics struct {
...
func newMetrics(st store.Store) *metrics {
	m := &metrics{
		...
		revocations: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Help: "Admin credential revocations by result (§7: revocation = store delete).",
		}, []string{"result"}),
		...
		verifyDuration: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			...
			Help: "Wall time of the pure verification step by lane (§11.5 target: p99 < 1 ms).",
