# Live-Stack Verification Record
...
`OzoneNativeAuthorizer` (§9.1). S3G publishes no host port; the proxy is the
only ingress (§7).

## Day-0 verdict — the synthetic header (§6.4, §11.1#1)
...
2. `ozone sh bucket info /s3v/<bucket>` → `owner: alice` for a bucket
   created via `aws s3 mb` through the proxy (e2e check §11.2).
...
recorded in §9.5 and tracked in [UPSTREAM.md](UPSTREAM.md).

## Finding — CreateBucket requires volume WRITE (2.1.1)
...
Fixed: the bootstrap grants `rwlc` on `/s3v`, DESIGN.md §9.2 amended. `rl` remains the floor for pre-provisioned
buckets.

## M1 exit criteria (§11.3) — e2e suite result
...
## M2 progress — presigned URLs (2026-07-06, same stack)
...
## M2 progress — multipart matrix (2026-07-06, same stack)
...
is the sufficient grant**, as §9.2 predicted. Owner flow verified end to
...
## M2 progress — human credential UX (2026-07-07, same stack)
...
- **`ozone-login` (device flow)** — the real binary, run on the compose
  ...
  consent, wrote the 0600 token file and printed the §6.9 recipe. The
...
## M2 progress — second issuer (2026-07-07, same stack)
...
## M2 complete — client smoke tests (2026-07-07, same stack)
...
- **boto3** (python:3.12-slim, current boto3): credentials resolved by
  ...
  and round-tripped an object: the §6.9 recipe proven with a real SDK.
- **mc** (minio/mc): `MC_HOST_<alias>` with the session token embedded in
  the URL; `mc --debug` shows the 8 MiB upload went out as
  `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` (aws-chunked) — the M2 "streaming
...
Every §11.4 item is now verified live — **M2 is complete**. Suite total:
**71 passed, 0 failed**.

## M3 complete — hardening (2026-07-08, same stack + HA/monitoring overlays)

All §11.5 exit criteria are verified live: valkey store + two replicas, the
...
encryption, §6.5); replica A forwards in `rewrite` mode, replica B in `resign`.
...
### Load test — verification overhead (§11.5 exit criterion)
...
endpoint returns 404 for an unknown AKID; and all three §7 NetworkPolicies
...
## M3 security review (2026-07-08)

A read-only review of the M3 surface (resign signer, valkey store + key
...
- **Admin-port exposure** — M3 added a state-changing, unauthenticated
...
- `resign` mode signs with a public constant, so it provides parser-robustness,
  **not** upstream authentication under a future Ozone secure mode (§6.4).
...
Deferred at review time, **both since fixed** (§7):
